...
export async function geminiTextPrompt(input: string) {
    try {
        ...
        console.log(`encrypted_API_KEY: ${JSON.stringify(encrypted_API_KEY?.API_Key)}`)
...
export async function getImgAIPrompt({ prompt, style, width, height, output_format, response_format }: QueryParams) {
    try {
        const session = await getServerSession(authOptions)
        const userId = Number(session.user.id)
        console.log(`Use ID: ${userId}`)
        ...
        const res = await axios({
            ...
            method: "POST",
            data: {
                query: { prompt, style, width, height, output_format, response_format, API_KEY: decrypted_API_KEY }
            }
        })
        console.log(res.data)
